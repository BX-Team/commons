...
public class PaperScheduler implements Scheduler {
    ...
    public Task runTask(Runnable runnable) {
        return new PaperSchedulerTask(globalRegionScheduler.run(plugin, task -> runnable.run()));
    }
    ...
    public Task runTaskLater(Runnable runnable, long delay) {
        ...
        return new PaperSchedulerTask(globalRegionScheduler.runDelayed(plugin, task -> runnable.run(), delay));
    }

    @Override
    public Task runTaskTimer(Runnable runnable, long delay, long period) {
        delay = getOneIfNotPositive(delay);
        return new PaperSchedulerTask(globalRegionScheduler.runAtFixedRate(plugin, task -> runnable.run(), delay, period));
    }

    @Override
    public Task runTask(Location location, Runnable runnable) {
        return new PaperSchedulerTask(regionScheduler.run(plugin, location, task -> runnable.run()));
    }
    ...
    public Task runTaskLater(Location location, Runnable runnable, long delay) {
        ...
        return new PaperSchedulerTask(regionScheduler.runDelayed(plugin, location, task -> runnable.run(), delay));
    }

    @Override
    public Task runTaskTimer(Location location, Runnable runnable, long delay, long period) {
        delay = getOneIfNotPositive(delay);
        return new PaperSchedulerTask(regionScheduler.runAtFixedRate(plugin, location, task -> runnable.run(), delay, period));
    }

    @Override
    public Task runTask(Entity entity, Runnable runnable) {
        return new PaperSchedulerTask(entity.getScheduler().run(plugin, task -> runnable.run(), null));
    }
    ...
    public Task runTaskLater(Entity entity, Runnable runnable, long delay) {
        ...
        return new PaperSchedulerTask(entity.getScheduler().runDelayed(plugin, task -> runnable.run(), null, delay));
    }

    @Override
    public Task runTaskTimer(Entity entity, Runnable runnable, long delay, long period) {
        delay = getOneIfNotPositive(delay);
        return new PaperSchedulerTask(entity.getScheduler().runAtFixedRate(plugin, task -> runnable.run(), null, delay, period));
    }

    @Override
    public Task runTaskAsynchronously(Runnable runnable) {
        return new PaperSchedulerTask(asyncScheduler.runNow(plugin, task -> runnable.run()));
    }

    @Override
    public Task runTaskLaterAsynchronously(Runnable runnable, long delay) {
        delay = getOneIfNotPositive(delay);
        return new PaperSchedulerTask(asyncScheduler.runDelayed(plugin, task -> runnable.run(), delay * 50L, TimeUnit.MILLISECONDS));
    }

    @Override
    public Task runTaskTimerAsynchronously(Runnable runnable, long delay, long period) {
        return new PaperSchedulerTask(asyncScheduler.runAtFixedRate(plugin, task -> runnable.run(), delay * 50, period * 50, TimeUnit.MILLISECONDS));
    }
